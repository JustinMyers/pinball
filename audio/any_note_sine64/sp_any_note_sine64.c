...
volatile uint8_t SvNextPWMValue;  // value for the PWM (Pulse Width Modulation) timer that makes the analog voltage for the output waveform, at 78,125 times per second.
...
void SfToneGenerator (struct NoteStruct SvNoteForPlayArray) {
	// changing from the original, which had as a base lookup table, a 256 step sine wave; to this version which uses as a base lookup table, a 64 step sine wave.
	// the SvPlayArray[][] waves are now 33-64 steps long (they were 129-256 steps long).
	// the 0 octave is now notes notes 72-81.
	...
	if (SvNoteForPlayArray.SvNoteNum < 72) {  // low note version, notes 0-71.
		// play each entry in SvPlayArray[] for two or more (2, 4, 8, 16, 32 or 64) 78K step(s) each.  SvPlayArray[] contains one sine wave.
		
		SvNoteHighNOTLow[SvNextLiveArr] = 0;  // set to false, this is a low note, not a high note
		SvNoteFactor[SvNextLiveArr] = ( 83 - SvNoteForPlayArray.SvNoteNum ) / 12;  // (1 << SvNoteFactor[]) is the number of 78K steps per entry in SvPlayArray[] (count by (1 << SvNoteFactor[]) when playing back
		...
		for ( SvArrayCount = 0; SvArrayCount <= SvArrayElements[SvNextLiveArr]; SvArrayCount++ ) {
			...
			;
			SvPlayPhaseArray[SvNextLiveArr][SvArrayCount] =  temp1 / ( SvArrayElements[SvNextLiveArr] + 1 ) ;  // phase in the Sine Wave of the current step
	...
	else {  // high note version, SvNoteNum >= 72.  notes 72-121.
		// play each entry in SvPlayArray[] for every 78K step.  SvPlayArray[] contains one or more (1, 2, 4, 8 or 16) sine waves.
		
		SvNoteHighNOTLow[SvNextLiveArr] = 1;  // set to true, this is a high note, not a low note
		SvNoteFactor[SvNextLiveArr] = ( (SvNoteForPlayArray.SvNoteNum - 72) / 12 );  // (1 << SvNoteFactor) is number of Sine Wave Cycles in the SvPlayArray
		SvArrayElements[SvNextLiveArr] = SvNoteWaveLength78K[SvNoteForPlayArray.SvNoteNum];
		for (SvArrayCount = 0; SvArrayCount <= SvArrayElements[SvNextLiveArr]; SvArrayCount++) {
			...
			SvPlayPhaseArray[SvNextLiveArr][SvArrayCount] =  (temp2 - temp4) << SvNoteFactor[SvNextLiveArr] ;  // phase in the Sine Wave of the current step
			SvPlayArray[SvNextLiveArr][SvArrayCount] = (SvSineWaveLookup256[ SvPlayPhaseArray[SvNextLiveArr][SvArrayCount] ] * SvNoteForPlayArray.SvNoteVolume) >> 8;
		}
	}
}
